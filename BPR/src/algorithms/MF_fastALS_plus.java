 ...
 * @author xiangnanhe
 */
public class MF_fastALS_plus extends TopKRecommender {
	/** Model priors to set. */
  ...
  double parad;
	...
	public MF_fastALS_plus(SparseMatrix trainMatrix, ArrayList<Rating> testRatings, 
			...
			SparseMatrix viewMatrix,double parad) {
		...
		this.viewmatrix = new SparseMatrix(viewMatrix);
		...
		Wi_new = new double[itemCount];
		Wneg = new double[itemCount];
		for (int i = 0; i < itemCount; i ++)
			Wi[i] = w0 * p[i] / Z;
		...
		//create view W
		this.parad = parad;
//		Wneg = new SparseMatrix(userCount, itemCount);
//		for (int u = 0; u < userCount; u ++)
//			for (int i : viewMatrix.getRowRef(u).indexList())
//				//Wneg.setValue(u, i, 1);
//				Wneg.setValue(u, i, parad+(1-parad)*Wi[i]);		
		for (int i = 0; i < itemCount; i++)
			Wneg[i] =  parad+(1-parad)*Wi[i];	
	}
	
	public MF_fastALS_plus(SparseMatrix trainMatrix, ArrayList<Rating> testRatings, 
			int topK, int threadNum, int factors, int maxIter, double w0, double alpha, double reg, 
			double init_mean, double init_stdev, boolean showProgress, boolean showLoss,
			SparseMatrix viewMatrix,double parad,int showbound,int showtime) {
		super(trainMatrix, testRatings, topK, threadNum);
		this.factors = factors;
		this.maxIter = maxIter;
		this.w0 = w0;
...
//				Wneg.setValue(u, i, parad+(1-parad)*Wi[i]);		
	...
	public void buildModel() {		
		...
		for (int iter = 0; iter < maxIter; iter ++) {
			...
			//System.out.printf("%d",showbound);
			// Show progress
			if (showProgress)
				if(iter >= showbound  &  iter%showtime ==0) {
	...
	protected void update_user(int u) {
		...
		ArrayList<Integer> viewList = null;
		if(viewmatrix.getRowRef(u) != null)
			viewList = viewmatrix.getRowRef(u).indexList();		
...
//				double val = 0;
//				for (int i = 0; i < itemCount; i ++) 
//					val += V.get(i, f) * V.get(i, k) * Wi_new[i];
//				SV.set(f, k, val);
//				SV.set(k, f, val);
//			}
//		}
		SVu = new  DenseMatrix(SV);
		for (int f = 0; f < factors; f++) {
			for (int k = 0;k<=f;k++) {
				double val = SV.get(f, k);
				for (int i: viewList) {
					val += V.get(i, f) * V.get(i, k) * parad * (1 - Wi[i]);
				}
				SVu.set(f,k,val);
				SVu.set(k,f,val);
			}
		...
		for ( int f = 0; f < factors; f ++) {
			...
			for (int i : itemList) {
				...
				numer +=  (w_items[i]*rating_items[i] - (w_items[i]-Wi_new[i]) * prediction_items[i]) * V.get(i, f);
				denom += (w_items[i]-Wi_new[i]) * V.get(i, f) * V.get(i, f);
			}
			denom += SVu.get(f, f) + reg;
	...
	protected void update_item(int i) {
		...
		ArrayList<Integer> viewList = viewmatrix.getColRef(i).indexList();
...
//			for (int i = 0;i < itemCount; i++ ) {
//				double pred = predict(u, i);
//				if (trainMatrix.getRowRef(u).indexList().contains(i))
//					l += W.getValue(u, i) * Math.pow(trainMatrix.getValue(u, i) - pred, 2);
//				else if (viewmatrix.getRowRef(u).indexList().contains(i))
//					l += Wneg.getValue(u, i) * Math.pow(pred, 2);
//				else 
//					l += Wi[i] * Math.pow(pred, 2);		
//			}
