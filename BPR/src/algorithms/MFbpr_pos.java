 ...
 *
 */
public class MFbpr_pos extends TopKRecommender {
	/** Model priors to set. */
  ...
  Random rand = new Random();
	public MFbpr_pos(SparseMatrix trainMatrix, ArrayList<Rating> testRatings,
			...
			int showbound ,int showtime, SparseMatrix viewMatrix, double d ) {
	...
	public void buildModel() {	
		...
		for (int iter = 0; iter < maxIter; iter ++) {
			...
			for (int s = 0; s < nonzeros; s ++) { 
				...
				if( parad < p) {				
				if (itemList.size() == 0)	{System.out.print(u);continue;}
				...
				update_ui(u, i);
					...
					ipvcount ++;
					itemList.addAll(viewList);
					if (itemList.size() == 0)	{System.out.print(u);continue;}
					int i = itemList.get(rand.nextInt(itemList.size()));
					update_ui_pos(u, i, itemList);
				}
			}
			...
			System.out.printf("train sample =  %d , ipv sample = %d\n", 
					traincount,ipvcount);
	...
	private void update_ui_pos(int u, int i, ArrayList<Integer> itemList) {
		// sample a negative item (uniformly random)
		int j = rand.nextInt(itemCount);
		
		while (itemList.contains(j)) {
			j = rand.nextInt(itemCount);
		}
